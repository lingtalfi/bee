/**
 * @depends jquery, pea, jqueryui, jutil
 *
 */
    ...
    (function () {
        ...
        window.beauty = function (params) {
            ...
            this.loadTemplateWithJsonP = function (tplId, jContainer, callback) {
                ...
                var jsonServerUrl = beautyUrl + '/server/json.php?tpl=' + tplId + '&callback=?';
        ...
        function findGroupByDotPath(jContainer, path) {
            var ret = jContainer.find('.testgroup-item[data-path=' + jUtil.selectorEscape(path) + ']:first');
            if (ret.length) {
                return ret;
            }
            return false;
        ...
        function initGroups(jContainer) {
            jContainer.find('.testgroup-item').each(function () {
                ...
                var path = getDotPathByGroup($(this));
                $(this).attr('data-path', path);
            });
        }

        function getDotPathByGroup(jGroup) {
            var paths = [];
