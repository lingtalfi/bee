 ...
 * What's new?
 *
 * Handling of html brackets syntax (for instance sports[fight], sports[][fight], ...)
 * 
 * - 1.05: Fix returned static values when collected. *
        ...
        window.beef = {
            ...
            form: function (options) {
                ...
                function collectElementsAndValues() {
                    ...
                    for (var i in vvv) {
                        //var staticName = noEndingBrackets(vvv[i].name);
                        allElements[vvv[i].name] = true;
                        injectValueByHtmlPath(vvv[i].name, vvv[i].value, values);
                    }
                    
                    // now collect dynamic elements
                    for (var name in dynamicElements) {
                        allElements[name] = dynamicElements[name];
                        injectValueByHtmlPath(name, dynamicElements[name].getValue(), values);
                    }
                }
                 ...
                 */
                function injectValueByHtmlPath(htmlKey, value, $values) {
                    var p = htmlKey.split('[]');
