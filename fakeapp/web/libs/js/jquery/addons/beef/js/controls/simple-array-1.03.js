 ...
 * What's new ?
 *
 * 2015-02-01: fix clearing the root array updates the counter to 0
 *
 * - 1.02: 2015-01-30: fix moving (grab) elements results not working properly
 * - 1.01: 2015-01-29: fixing: returned value incorrect with numeric keys (added util.objectify method)
 *
 * - 1.0
 *
 *
    ...
    (function () {
        ...
        var util = {
            ...
            getParentPath: function (path) {
                return window.bdot.getParentKeyPath(path);
            },
            ...
            resetChildrenOrder: function (jParentItem, oSimpleArray, parentPath, oldPath) {
                /**
                 * @1.02
                 * We want to reorder the items, so that we can really sort them (choose the order) as we want.
                 * However, due to javascript nature, it only work for associative indexes (won't work with numerical indexes)
                 */
                var newValue = {};
                gui.getItemChildren(jParentItem, oSimpleArray).each(function () {
                    var key = gui.getKey($(this));
                    var path = util.concatenatePath(parentPath, key);
                    newValue[key] = oSimpleArray._getValue(path);
                });
                oSimpleArray._unsetValue(oldPath);
                oSimpleArray._setValue(parentPath, newValue);
            },
        ...
        var gui = {
            assignSortable: function (jEl, oSimpleArray) {
                ...
                jEl.sortable({
                    ...
                    stop: function (e, ui) {
                        ...
                        var oldParentPath = util.getParentPath(oldPath);
                        ...
                        if (true === isValid) {
                            ...
                            if (oldParentPath !== parentPath) {
                                 ...
                                 */
                                var oldValCpy;
                                if (pea.isArrayOrObject(oldValue)) {
                                    oldValCpy = $.extend(true, {}, oldValue);
                                }
                                else {
                                    oldValCpy = oldValue;
                                }
                            ...
                            util.resetChildrenOrder(jParentItem, oSimpleArray, parentPath, oldPath);
            ...
            getItemChildren: function (jParentItem, oSimpleArray) {
                var params = oSimpleArray.getParams();
                var jContainer = params.getContainer(jParentItem);
                return jContainer.find('> .' + cssItem);
            },
                ...
                jRoot.on('click.simplearray', function (e) {
                    ...
                    if (jTarget.hasClass('toggle')) {
                        ...
                        if (true === gui.isRoot(jItem)) {
                            gui.updateCounterByItem(jItem, 0, 0);
