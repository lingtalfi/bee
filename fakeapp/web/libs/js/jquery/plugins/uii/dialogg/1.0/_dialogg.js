(function ($) {
    ...
    function startDialogg(jContentContent, options) {
        ...
        // create the dialogg
        var jDialog = $(options.buildDialogg());
...
//        }
        ...
        if (true === options.resizable && ('resizz' in $.fn)) {
            var contentWidth = jContent.width();
            var contentHeight = jContent.height();
            jDialog.resizz({
                resize: function (props, width, height, offsetX, offsetY) {
                    var deltaWidth = props.width - width;
                    var deltaHeight = props.height - height;
                    jContent.css({
                        width: deltaWidth + contentWidth,
                        height: deltaHeight + contentHeight
                    });
                },
        ...
        if ('option' === args[0]) {
            ...
            if (args[1]) {
                ...
                if (jDialog.length) {
                    ...
                    if (null !== width) {
                        props.width = width;
                        props.left = jDialog.offset().left + Math.abs((jDialog.width() - width) / 2);
                    }
                    if (null !== height) {
                        props.height = height;
                        var contentHeight = jContent.height() + (height - jDialog.height());
                        jContent.animate({
